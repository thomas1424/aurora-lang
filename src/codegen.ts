...
export function generateJS(ast: AST): string {
  if (ast.kind === 'Program') {
    return ast.body.map(b => generateJS(b)).join('\n');
  }
  if (ast.kind === 'NumberLiteral') return ast.value;
  if (ast.kind === 'StringLiteral') return JSON.stringify(ast.value);
  ...
  if (ast.kind === 'ExprStmt') return `${generateJS(ast.expr)};`;
  if (ast.kind === 'Call') {
  ...
  if (ast.kind === 'Return') return `return ${ast.arg ? generateJS(ast.arg) : ''};`;
  if (ast.kind === 'If') {
  ...
  throw new Error('Unknown AST node in codegen: ' + (ast as any).kind);
}
