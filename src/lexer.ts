...
export function tokenize(src: string): Token[] {
  ...
  while (i < src.length) {
    ...
    if (/[0-9]/.test(c)) {
      let n = c; i++;
      while (/[0-9.]/.test(src[i])) { n += src[i]; i++; }
    ...
    if (c === '"' || c === "'") {
      ...
      while (i < src.length && src[i] !== quote) {
        if (src[i] === '\\') { i++; if (i < src.length) { s += src[i]; i++; } }
        else { s += src[i++]; }
      }
      i++; // skip quote
    ...
    if (/[A-Za-z_]/.test(c)) {
      let id = c; i++;
      while (/[A-Za-z0-9_]/.test(src[i])) { id += src[i++]; }
    ...
    if (['==','!=','<=','>=','&&','||'].includes(two)) {
      tokens.push({type: two, text: two}); i+=2; continue;
    }
    tokens.push({type: c, text: c}); i++;
  }
